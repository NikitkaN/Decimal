...
void scale_equalization(s21_decimal *value_1, s21_decimal *value_2) {
  if (get_scale(value_1) < get_scale(value_2)) {
    ...
    if (getbits(*value_2, 93) == 0 && getbits(*value_2, 94) == 0 &&
        getbits(*value_2, 95) == 0) {
    ...
    if (getbits(*value_1, 93) == 0 && getbits(*value_1, 94) == 0 &&
        getbits(*value_1, 95) == 0) {
      for (int i = 0; i < difference; i++) {
        mul_ten(value_2);
      }
      set_scale(value_2, get_scale(value_1));
...
int s21_is_less(s21_decimal value_1, s21_decimal value_2) {
  ...
  int sign_1 = getsigns(value_1);
  int sign_2 = getsigns(value_2);
  if (get_scale(&value_1) != get_scale(&value_2))
...
int s21_is_equal(s21_decimal value_1, s21_decimal value_2) {
  ...
  if (is_equal_without_sign(zero, value_1) == TRUE &&
    ...
    return_value = TRUE;
  } else if (getsigns(value_1) != getsigns(value_2)) {
    return_value = FALSE;
...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (sign_1 == sign_2) {
    if (sign_1 == 0) {  //                                                a + b
      return_value = bits_add(value_1, value_2, result);
      setsigns(result, 0);
    } else if (sign_1 ==
               1) {  //                                         (-a) + (-b)
      if ((return_value = bits_add(value_1, value_2, result)) == INF)
        return_value = N_INF;
      setsigns(result, 1);
    }
  } else if (sign_1 <
...
void bit_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result,
  ...
  for (int i = last_bit(value_1); i >= 0; i--) {
    if (getbits(value_1, i)) setbits(tmp, 0, 1);
    if (s21_is_greater_or_equal(*tmp, value_2) == TRUE) {
      ...
      offset_left(result, 1);
      if (i != 0) offset_left(tmp, 1);
      if ((i - 1) >= 0 && getbits(value_1, i - 1)) setbits(tmp, 0, 1);
    }
  }
}
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  int res = OK;
  int sign = getsigns(src);
  int s = getbits(src, 31);
...
int getbits(s21_decimal num, int bit) {
  int res = 0;
  if ((bit / 32) < 4) {
...
void cleanbits(s21_decimal *num) {
  if (num != NULL) {
...
int last_bit(s21_decimal number) {
  int last_bit = 95;
  for (; last_bit >= 0 && getbits(number, last_bit) == 0; last_bit--) {
  }
  return last_bit;
}

void setbits(s21_decimal *num, int bit, int value) {
  unsigned int mask = 1u << (bit % 32);
...
int getsigns(s21_decimal num) {
  unsigned int mask = 1u << 31;
  int res = num.bits[3] & mask;
  return !!res;
}

void setsigns(s21_decimal *num, int sign) {
  unsigned int mask = 1u << 31;
  if (sign != 0) {
    num->bits[3] |= mask;
  } else {
...
void set_scale(s21_decimal *num, int scale) {
  ...
  num->bits[3] &= cleanMask;
  int mask = scale << 16;
  num->bits[3] |= mask;
}

int bits_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  cleanbits(result);
  int return_value = OK;
  int tmp = 0;
  int i = 0;
  for (; i < 96; i++) {
    int cur_bit_val_1 = getbits(value_1, i);
    int cur_bit_val_2 = getbits(value_2, i);
    if (!cur_bit_val_1 && !cur_bit_val_2) {
      if (tmp == 1) {
        setbits(result, i, TRUE);
        tmp = 0;
      } else {
        setbits(result, i, FALSE);
      }
  ...
  if (return_value == INF) cleanbits(result);
  return return_value;
}
...
int bit_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  for (int i = 0; i <= lastBit; i++) {
    cleanbits(&tmp);
    int cur_val_1 = getbits(value_1, i);
    if (cur_val_1) {
      tmp = value_2;
...
int offset_left(s21_decimal *num, int value_offset) {
  ...
  if (lastbit + value_offset > 95) {
    ...
    for (int i = 0; i < value_offset; i++) {
      int value_31bit = getbits(*num, 31);
      int value_63bit = getbits(*num, 63);
      num->bits[0] <<= 1;
      num->bits[1] <<= 1;
      num->bits[2] <<= 1;
      if (value_31bit) setbits(num, 32, 1);
      if (value_63bit) setbits(num, 64, 1);
    }
  }
  return return_value;
}
